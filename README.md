...
Nesse plano é interessante notar o comportamento da tangente(*Linha Vermelha*) em comparação com o seno/cosseno. O valor da tangente em h(x) = $`\frac{seno}{cosseno}`$, e em todos os momentos onde $`\frac{seno}{cosseno}`$ = $`\frac{|1|}{0}`$, forma-se a assíntota, pois o resultado de um número dividido por 0 não existe, e nos momentos em que $`\frac{seno}{cosseno}`$ = $`\frac{0}{|1|}`$, o valor é zero, ou seja, a linha corta h(x)*A Linha vertical* onde o valor é 0.
